...
// 	int32_t status;
// 	print_message_fields(&requestHeader);
// 	if (sys_read32(0x4a60d000) & RT_THREAD_STATUS_ERROR_MASK) {
// 		LOG_ERR("Thread is corrupted, cannot send data.\n");
// 		status = -EINVAL;
// 	} else {
// 		status = sys_read32(0x4a60d000);
// 	}
// 	printk("Thread status:%x\n", status);
