...
static uint8_t rx_message_buf[MAILBOX_MBOX_SIZE];
...
struct tisci_data {
	...
	struct tisci_desc desc;
	struct tisci_version_info version;
	uint32_t host_id;
	uint8_t seq;
...
static int tisci_get_response(const struct device *dev, struct tisci_xfer *xfer)
{
	...
	if (k_sem_take(&response_ready_sem, K_MSEC(dev_data->desc.max_rx_timeout_ms)) != 0) {
		LOG_ERR("Timeout waiting for response");
		return -ETIMEDOUT;
	}

	if (xfer->rx_message.size > dev_data->desc.max_msg_size) {
		LOG_ERR("rx_message.size [ %d ] > max_msg_size\n", xfer->rx_message.size);
		return -EINVAL;
	}

	if (rx_message.size < xfer->rx_message.size) {
		LOG_ERR("rx_message.size [ %d ] < xfer->rx_message.size\n", rx_message.size);
		return -EINVAL;
	}

	memcpy(xfer->rx_message.buf, rx_message.buf, xfer->rx_message.size);
...
static int tisci_init(const struct device *dev)
{
	...
	rx_message.buf = rx_message_buf;
	rx_message.size = sizeof(rx_message_buf);

	k_sem_init(&response_ready_sem, 0, 1);
	data->host_id = config->host_id;
	data->seq = 0x0;

	k_sem_init(&data->data_sem, 1, 1);

	data->desc.max_msg_size = MAILBOX_MBOX_SIZE;
	/* Conservative timeout */
	data->desc.max_rx_timeout_ms = 10000;
			      ...
			      uint8_t *programmed_state, uint8_t *current_state)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_CLOCK_STATE, 0, &req,
			  ...
			  uint8_t state)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_CLOCK_STATE, flags, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
			     ...
			     uint8_t parent_id)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_CLOCK_PARENT, 0, &req,
				    sizeof(req), &resp, sizeof(resp));
			     ...
			     uint8_t *parent_id)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_CLOCK_PARENT, 0, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
				  ...
				  uint8_t *num_parents)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_NUM_CLOCK_PARENTS, 0, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
...
int tisci_set_device_state(const struct device *dev, uint32_t dev_id, uint32_t flags, uint8_t state)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_DEVICE_STATE, flags, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
				   ...
				   uint8_t state)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_DEVICE_STATE,
				    flags | TISCI_FLAG_REQ_GENERIC_NORESPONSE, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
			   ...
			   uint32_t *resets, uint8_t *p_state, uint8_t *c_state)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_DEVICE_STATE, 0, &req,
				    sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
...
int tisci_cmd_proc_request(const struct device *dev, uint8_t proc_id)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_PROC_REQUEST, 0, &req, sizeof(req),
				    &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
...
int tisci_cmd_proc_release(const struct device *dev, uint8_t proc_id)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_PROC_RELEASE, 0, &req, sizeof(req),
				    &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
...
int tisci_cmd_proc_handover(const struct device *dev, uint8_t proc_id, uint8_t host_id)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_PROC_HANDOVER, 0, &req, sizeof(req),
				    &resp, sizeof(resp));
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
				...
				uint32_t config_flags_set, uint32_t config_flags_clear)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_PROC_BOOT_CONFIG, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_set_proc_boot_cfg: Failed to setup transfer");
        return -EINVAL;
    }
				 ...
				 uint32_t control_flags_set, uint32_t control_flags_clear)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SET_PROC_BOOT_CTRL, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_set_proc_boot_ctrl: Failed to setup transfer");
        return -EINVAL;
    }
				   ...
				   uint32_t *image_size)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_PROC_AUTH_BOOT_IMAGE, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_proc_auth_boot_image: Failed to setup transfer");
        return -EINVAL;
    }
	...
	if (image_addr) {
		*image_addr = (resp.image_addr_low & TISCI_ADDR_LOW_MASK) |
			      (((uint64_t)resp.image_addr_high << TISCI_ADDR_HIGH_SHIFT) &
				   ...
				   uint32_t *cfg_flags, uint32_t *ctrl_flags, uint32_t *sts_flags)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_PROC_BOOT_STATUS, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_get_proc_boot_status: Failed to setup transfer");
        return -EINVAL;
    }
			     ...
			     uint8_t s_host, uint16_t *range_start, uint16_t *range_num)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_GET_RESOURCE_RANGE, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_get_resource_range: Failed to setup transfer");
        return -EINVAL;
    }
				   ...
				   uint32_t size)
{
	...
	xfer = tisci_setup_one_xfer(dev, msg_type, 0, &req, sizeof(req), &resp,
                                sizeof(resp));
    if (!xfer) {
        LOG_ERR("cmd_set_board_config_using_msg: Failed to setup board config transfer");
        return -EINVAL;
    }
...
int tisci_cmd_get_revision(const struct device *dev, struct tisci_version_info *ver)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_VERSION, 0, &hdr, sizeof(hdr), &rev_info,
                                sizeof(rev_info));
    if (!xfer) {
        LOG_ERR("tisci_cmd_get_revision: Failed to setup transfer");
        return -EINVAL;
    }
...
int tisci_cmd_sys_reset(const struct device *dev)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_SYS_RESET, 0, &req, sizeof(req),
                                &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_sys_reset: Failed to setup system reset transfer");
        return -EINVAL;
    }
...
int tisci_cmd_query_msmc(const struct device *dev, uint64_t *msmc_start, uint64_t *msmc_end)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_QUERY_MSMC, 0, &req, sizeof(req),
                                &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_query_msmc: Failed to setup MSMC query transfer");
        return -EINVAL;
    }
...
int tisci_cmd_set_fwl_region(const struct device *dev, const struct tisci_msg_fwl_region *region)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_FWL_SET, 0, &req, sizeof(req), &resp,
                                sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_set_fwl_region: Failed to setup firewall config transfer");
        return -EINVAL;
    }
...
int tisci_cmd_get_fwl_region(const struct device *dev, struct tisci_msg_fwl_region *region)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_FWL_GET, 0, &req, sizeof(req), &resp,
                                sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_get_fwl_region: Failed to setup firewall query transfer");
        return -EINVAL;
    }
...
int tisci_cmd_change_fwl_owner(const struct device *dev, struct tisci_msg_fwl_owner *owner)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_FWL_CHANGE_OWNER, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_change_fwl_owner: Failed to setup firewall owner change transfer");
        return -EINVAL;
    }
				 ...
				 const struct tisci_msg_rm_udmap_tx_ch_cfg *params)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_UDMAP_TX_CH_CFG, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_udmap_tx_ch_cfg: Failed to setup UDMAP TX channel config transfer");
        return -EINVAL;
    }
				 ...
				 const struct tisci_msg_rm_udmap_rx_ch_cfg *params)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_UDMAP_RX_CH_CFG, 0, &req,
                                sizeof(req), &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_udmap_rx_ch_cfg: Failed to setup UDMAP RX channel config transfer");
        return -EINVAL;
    }
			   ...
			   uint32_t dst_thread)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_PSIL_PAIR, 0, &req, sizeof(req),
                                &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_psil_pair: Failed to setup PSI-L pair transfer");
        return -EINVAL;
    }
			     ...
			     uint32_t dst_thread)
{
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_PSIL_UNPAIR, 0, &req, sizeof(req),
                                &resp, sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_psil_unpair: Failed to setup PSI-L unpair transfer");
        return -EINVAL;
    }
...
static int tisci_cmd_rm_irq_set_raw(const struct device *dev,
				    struct tisci_msg_rm_irq_set_req_wrapper *req)
{
	struct tisci_xfer *xfer;
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_IRQ_SET, 0, req, sizeof(*req), &resp,
                                sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_irq_set_raw: Failed to setup transfer");
        return -EINVAL;
    }

	int ret = tisci_do_xfer(dev, xfer);
	...
	if ((resp.hdr.flags & TISCI_FLAG_RESP_GENERIC_ACK) != TISCI_FLAG_RESP_GENERIC_ACK) {
		return -EIO;
	}
...
int tisci_cmd_rm_irq_set(const struct device *dev, struct tisci_msg_rm_irq_set_req *req)
{
	if (!req) {
		return -EINVAL;
	}

	struct tisci_msg_hdr hdr = {0};
	struct tisci_msg_rm_irq_set_req_wrapper req_wrapped = {.req = req, .hdr = hdr};

	return tisci_cmd_rm_irq_set_raw(dev, &req_wrapped);
}

static int tisci_cmd_rm_irq_release_raw(const struct device *dev,
					struct tisci_msg_rm_irq_release_req_wrapper *req)
{
	struct tisci_xfer *xfer;
	...
	xfer = tisci_setup_one_xfer(dev, TISCI_MSG_RM_IRQ_RELEASE, 0, req, sizeof(*req), &resp,
                                sizeof(resp));
    if (!xfer) {
        LOG_ERR("tisci_cmd_rm_irq_release_raw: Failed to setup transfer");
        return -EINVAL;
    }
...
int tisci_cmd_rm_irq_release(const struct device *dev, struct tisci_msg_rm_irq_release_req *req)
{
	if (!req) {
		return -EINVAL;
	}
	...
	struct tisci_msg_hdr hdr;
	struct tisci_msg_rm_irq_release_req_wrapper req_wrapped = {.req = req, .hdr = hdr};

	return tisci_cmd_rm_irq_release_raw(dev, &req_wrapped);
}
