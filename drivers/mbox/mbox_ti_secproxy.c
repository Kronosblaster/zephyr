...
static int secproxy_mailbox_send(const struct device *dev, uint32_t channel, const struct mbox_msg *msg)
{
    struct secproxy_mailbox_data *data = DEV_DATA(dev);
    struct secproxy_thread spt;
    ...
    while (data_reg <= (spt.target_data + 0x3C)) {
...
#define MAILBOX_INSTANCE_DEFINE(idx)	\
    ...
    DEVICE_DT_INST_DEFINE(idx, secproxy_mailbox_##idx##_init, \
                 ...
                 &secproxy_mailbox_##idx##_config, PRE_KERNEL_1, \
                 CONFIG_MBOX_TI_SECURE_PROXY_PRIORITY,	\
