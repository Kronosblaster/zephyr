...
{
	int ret;
	ret = ti_sci_cmd_put_device(dmsc, pd->devid);
	if (ret) {
...
{
	struct power_domain *data = dmsc->data;
	int ret = 0;
...
{
	// Optionally, perform any TISCI-specific initialization here.
	return pm_device_runtime_enable(dev);
}

#define TISCI_PD_DEVICE_DEFINE(inst)                                                               \
	static struct power_domain power_domain_data_##inst = {                                    \
		.devid = DT_INST_PHA_BY_IDX(inst, power_domains, 0, devid),                             \
		.mode = DT_INST_PHA_BY_IDX(inst, power_domains, 0, mode),                           \
	}                                                                                         \
	PM_DEVICE_DT_INST_DEFINE(inst, tisci_pd_pm_action);                                        \
	DEVICE_DT_INST_DEFINE(inst, tisci_pd_init, PM_DEVICE_DT_INST_GET(inst),                    \
				  &power_domain_data_##inst, NULL,                                            \
				  PRE_KERNEL_1, CONFIG_POWER_DOMAIN_TISCI_INIT_PRIORITY, NULL);
